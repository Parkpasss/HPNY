...
import { BookingParamsProps, ActivityType } from "@/interface" // ActivityType 사용
...
export default async function ActivityBookingPage({
  ...
  const data: ActivityType = await getActivityData(id) // 활동 데이터 가져오기
...
async function getActivityData(id: string) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/activities?id=${id}`, // 활동 API 경로
      {
        cache: "no-store",
      },
    )
